...
var scp_prep = function() {
    ...
    // Internal Note Canned Responses.

  
    $('form select#cannedResp2').on('select2:opening', function (e) {
        var redactor = $('#internal_note', $(this).closest('form')).data('redactor');
        if (redactor)
            redactor.api('selection.save');
    });

    $('form select#cannedResp2').change(function() {
        ...
        $.ajax({
                ...
                success: function(canned){
                    ...
                    var box = $('#internal_note', fObj),
                        redactor = $R('#internal_note.richtext');
                    if (canned.response) {
                        if (redactor) {
                            redactor.api('selection.restore');
                            redactor.insertion.insertHtml(canned.response);
